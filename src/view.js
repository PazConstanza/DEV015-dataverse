...
export const renderItems = (data) => {
  const tarjetas = data.map(campeona => {
    ...
    //Se crea un array con la informacion a mmostrar
    const información = [campeona.nombre, campeona.descripcionCorta, campeona.hechos.frase, campeona.hechos.dificultadDeUso, campeona.hechos.tipoDeDano, campeona.hechos.carril];


    información.forEach((texto, index) => {               // Recorrer el array y crear un <li> para cada elemento
      const nuevoLi = document.createElement('li');      // Crear un nuevo elemento <li>
      nuevoLi.id = `elemento${index + 1}`;              // Asignar un ID único a cada <li>
      nuevoLi.textContent = texto;                     // Agregar texto al <li>
      tarjeta.appendChild(nuevoLi);                   // Añadir el <li> al <ul>
      console.log(nuevoLi);
    });
